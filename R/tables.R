summarize_monthly_table <- function(df = "", grouping = "", 
  ...
  caption.text = paste(year_select, "monthly summary table at the ", sampling_station, "location.")
  pander(table.values, justify = c("left", rep("center", ncol(table.values)-1)), 
         #style = "grid", 
...
group_table_historical <- function(df = "", year_select = "", grouping = "", 
  ...
  df <- df %>% filter(parm_tag == grouping & station == sampling_station)
  
  month_values <- summarize_monthly(labdat = df) %>% filter(month == month_select, year == year_select)
  ...
  if(by_month == T) {
    hist.values <- left_join(hist_bas, hist_ext) %>% left_join(hist_per) %>% filter(month == month_select)
    table.values <- left_join(month_values, hist.values)
    table.values <- as.data.frame(table.values)
    
    caption.text = paste(sampling_station, grouping, "characterics for", month_select, "historical data.")
    ...
    caption.text = paste("Overall historical summary for the", sampling_station, grouping, "characteristics.")
